...
const createOpamText = pkg => {
  const opamTemplate = [
    'opam-version: ' + quote("2.0"),
    'maintainer: ' + quote(pkg.author),
    'authors: [' + quote(pkg.author) + ']',
    'license: ' + quote(pkg.license),
    'homepage: ' + quote(pkg.homepage),
    'doc: ' + quote(pkg.homepage),
    pkg.repository && pkg.repository.url ?
      ('bug-reports: ' + quote(pkg.repository.url)) :
      '',
    pkg.repository && pkg.repository.url ?
      ('dev-repo: ' + quote(pkg.repository.url.replace('https', 'git').replace('http', 'git'))) :
      '',
    'tags: [' + (pkg.keywords ? pkg.keywords.map(quote).join(' ') : '') + ']',
    'build: [ [' + pkg.esy.build.split(' ').map(quote).join(' ') + ' ] ]',
    'depends: [',
  ].concat(depMap(pkg.dependencies).map(s=>'  ' + s)).concat([
    ']',
    'synopsis: ' +
      quote(pkg.description.charAt(0).toUpperCase() + pkg.description.substr(1)),
