...
module Make = (Config: FileContextPrinterConfig, Styl: Stylish.StylishSig) => {
  ...
  let _printFile =
    ...
    /* we display no more than 3 lines after startRow. Some endRow are rly far
       away */
    let displayedEndRow = min(List.length(content) - 1, startRow + Config.linesAfter);
  ...
  let print = (~path: string, ~range: rowColumnRange) => {
    let fileContents = Helpers.fileLinesOfExn(path);
    _printFile(range, fileContents)
    |> List.rev
    |> String.concat("\n");
  };
};
