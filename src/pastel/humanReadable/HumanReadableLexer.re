...
let humanReadableTokenToToken = terminalToken => {
    ...
    | RawEscapeSequence(token) =>
      EscapeSequence(
        ...
        | token when token == HumanReadable.color.black.start => Foreground(Black)
        | token when token == HumanReadable.color.red.start => Foreground(Red)
        | token when token == HumanReadable.color.green.start => Foreground(Green)
        | token when token == HumanReadable.color.yellow.start => Foreground(Yellow)
        | token when token == HumanReadable.color.blue.start => Foreground(Blue)
        | token when token == HumanReadable.color.magenta.start => Foreground(Magenta)
        | token when token == HumanReadable.color.cyan.start => Foreground(Cyan)
        | token when token == HumanReadable.color.white.start => Foreground(White)
        ...
        | token when token == HumanReadable.bg.yellow.start => Background(Yellow)
        | token when token == HumanReadable.bg.blue.start => Background(Blue)
        | token when token == HumanReadable.bg.magenta.start => Background(Magenta)
        ...
        | token when token == HumanReadable.bg.redBright.start => Background(BrightRed)
        | token when token == HumanReadable.bg.greenBright.start =>
          Background(BrightGreen)
        | token when token == HumanReadable.bg.yellowBright.start =>
          Background(BrightYellow)
        | token when token == HumanReadable.bg.blueBright.start => Background(BrightBlue)
        | token when token == HumanReadable.bg.magentaBright.start =>
          Background(BrightMagenta)
        | token when token == HumanReadable.bg.cyanBright.start => Background(BrightCyan)
        ...
        | token when token == HumanReadable.modifier.strikethrough.start => Strikethrough
